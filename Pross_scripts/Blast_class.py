...
class Blast:
    ...
    def run_psiblast(self):
        ...
        os.system('psiblast -query ' + self.infile_full \
                  + ' -db /shareDB/nr/latest/nr' \
                  + ' -evalue ' + self.evalue \
                  + ' -max_target_seqs ' + self.max_targets \
                  + ' -num_iterations 5'
    ...
    def output_in_fasta(self):
        ...
        for hit in parsed_hits:
            if float(hit[5]) >= float(self.min_id) and float(hit[3]) >= float(coverage):
                ...
                with open(self.note_file, 'a') as notes:
                    notes.write(str('hit failed ' + hit[0] + ' Id(%) = ' + str(hit[5])[0:5] + ' coverage ' + str(hit[3])) + '\n')
                    notes.close()
    ...
    def run_cd_hit(self, infile, clust_threshold): # CR: infile = blast_infile_out_filt_fa
        ...
        os.system('cd-hit -i ' + infile + ' -o ' + self.cd_hit_outfile + '_temp' + ' -c ' + clust_threshold)
