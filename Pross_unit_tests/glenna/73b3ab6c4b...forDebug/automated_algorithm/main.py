...
if my_query[9] == 'No':
    if my_query[6]:
        refine = Refinement(pose, symmetry)  #If desired, idealization is also called here.
    else:
        refine = Refinement(pose, symmetry)  #If desired, idealization is also called here.
...
#run filterscan
filterscan = Filterscan(pose, symmetry, input_for_fs, keep_native, res_to_restrict_str)
filterscan.filterscan()
filterscan.verify_output()

######################################################################################

design = Design(pose, symmetry, input_for_fs, keep_native, filterscan.get_resfiles())
design.design()
