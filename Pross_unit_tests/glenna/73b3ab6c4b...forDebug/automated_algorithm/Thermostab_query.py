...
class Thermostab_query:
    ...
    def __init__(self, args):
        ...
        if len(args[0]) == 4:
            ...
            self.pdbid = 'uploaded_pdb'
            self.uploaded_full_path = args[0]

        self.input_seq = args[1].replace(" ", "").replace("'\n'","")
        self.chain_id = args[2]
        self.small_molecules = args[3]
        self.interacting_chains = args[4]
        self.res_to_fix_str = args[5]
        self.oligomers = args[6]
        self.idealize = args[7]
        self.msa = args[8]
        self.need_refine = args[9]
        self.uploaded_fasta = args[10]
    ...
    def get_chain_identifiers(self):
        ...
        if self.uploaded_full_path:
            ...
            url_fasta = 'http://www.rcsb.org/pdb/files/fasta.txt?structureIdList=' + self.pdbid
            self.fasta_file_name = self.pdbid + ".fasta"
            urllib.urlretrieve(url_fasta, self.path_in + '/' + self.fasta_file_name)
    ...
    def extract_pdb_full_seq(self):
        #Extracting the full_sequence of the pdb structure from the rcsb display file options.
        #Make sure the pdb file does not contain BXXX residues (for example: BGLN)
        if self.path_in + '/' + self.fasta_file_name == None:
            url_full_seq = 'http://www.rcsb.org/pdb/files/fasta.txt?structureIdList=' + pdbid
            list_of_lines = urllib.urlopen(url_full_seq).readlines()
    ...
    def create_dirs(self):
        ...
        self.path_refinement = self.path + '/refinement'
        self.path_filterscan = self.path + '/filterscan'
        self.path_design = self.path + '/design'
        self.path_idealize = self.path + '/idealize'
        self.path_msa = self.path + '/msa'

        sub_dirs = [self.path_in, self.path_refinement, self.path_filterscan, self.path_design, self.path_idealize, self.path_msa]
        os.makedirs(self.path)
        for dir in sub_dirs:
            os.makedirs(os.path.join(self.path, dir))

        if self.idealize == 'No':
            os.system('rm -rf ' + self.path_idealize)

        #print the args from flag file used in the command line into a file.
        arg_vec = ['-pdbid=', '-input_seq=', '-chain_id=', '-small_ligands=', '-large_ligands=', \
                   '-res_to_fix=', '-symm_interfaces=', '-idealize=', '-input_msa=', '-refined_file=', '-uploaded_fasta=']
    ...
    def make_symm_file(self, input_pdb, in_out_path):
        self.symm_filename = str(self.pdbid) + '_C2.symm'
    ...
    def get_path_refinement(self):
        return self.path_refinement

    def get_path_idealize(self):
        return self.path_idealize

    def get_path_msa(self):
        return self.path_msa

    def get_path_fs(self):
        return self.path_filterscan

    def get_path_design(self):
        return self.path_design
    ...
    def get_coorcst_value(self):
        if 90 < self.homology_id <= 100:
            coorcst = 0.4
        elif 60 < self.homology_id <= 90:
            coorcst = 0.1
        else:
            coorcst = 0.02
    ...
    def get_res_to_fix(self):
        # res_to_fix_string = str(self.interacting_res_renumbered[0]) #special case: no comma before.
        # for i in range (1, len(self.interacting_res_renumbered)): #all other residues.
        #     res_to_fix_string += ',' + str(self.interacting_res_renumbered[i])
